...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Upload a technical debt classification model to Hugging Face Hub"
    )

    # Authentication arguments
    parser.add_argument(
        "--token", type=str, required=True, help="Hugging Face API token"
    )
    ...
    parser.add_argument(
        "--commit_message",
        type=str,
        default="Upload TD-Classifier model",
...
def generate_model_card(model_path: str) -> str:
    ...
    card_content += (
        "This model was trained using the TD-Classifier Suite "
        "for technical debt classification.\n\n"
    )
    ...
    if training_config:
        ...
        card_content += (
            f"- Base Model: {training_config.get('model_name', 'Unknown')}\n"
        )
        card_content += (
            f"- Training Dataset: {training_config.get('data_file', 'Unknown')}\n"
        )
        ...
        card_content += (
            f"- Max Sequence Length: {training_config.get('max_length', 'Unknown')}\n"
        )

        if training_config.get("cross_validation"):
            n_splits = training_config.get("n_splits", 5)
            card_content += f"- Cross-Validation: Yes ({n_splits} folds)\n"
        else:
    ...
    card_content += (
        "label = 'Technical Debt' if prediction == 1 else 'Not Technical Debt'\n"
...
def main():
    ...
    print("🔐 Authenticating with Hugging Face using the provided token...")
    ...
    if args.create_if_not_exists:
        try:
            ...
            print(
                f"🛠️ Creating new {args.repo_visibility} repository: "
                f"'{args.repo_name}'..."
            )
            create_repo(
    ...
    print(
        ...
        f"AutoModelForSequenceClassification.from_pretrained('{args.repo_name}')"
    )
