...
def load_dataset_from_huggingface(
    ...
    """
    try:
        # Imported lazily: `datasets` ships only with the [train]/[gpu] extras,
        # so the default ONNX-only install stays free of this heavy dependency.
        from datasets import load_dataset as hf_load_dataset
    except ImportError as exc:
        raise ImportError(
            "Loading a dataset from the Hugging Face Hub requires the "
            "'datasets' package. Install it with: pip install 'tdsuite[train]'"
        ) from exc
