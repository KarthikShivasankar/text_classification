...
class DataSplitter:
    ...
    def __init__(
        ...
        Args:
            ...
            positive_category: Category to treat as positive for binary tasks
        """
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.is_numeric_labels and self.positive_category is not None:
            ...
            df[self.label_column] = (
                df[self.label_column] == self.positive_category
            ).astype(int)
    ...
    def split_and_save(
        ...
        its data, removes it from the main dataset, then splits the remaining
        data into train and test. Finally, balances the top repository data by
        copying negative samples from test.csv if needed.
        ...
        if top_repo_df is not None:
            ...
            positive_count = len(
                top_repo_df[top_repo_df[self.label_column] == positive_label]
            )
            negative_count = len(
                top_repo_df[top_repo_df[self.label_column] != positive_label]
            )
            ...
            print(
                f"Top repository data: {positive_count} positive samples, "
                f"{negative_count} negative samples"
            )

            # If the top repository data is unbalanced, copy negatives from test.csv
            if positive_count > negative_count:
                # Get negative samples from test.csv
                test_negative = test_df[
                    test_df[self.label_column] != positive_label
                ].copy()
                ...
                if len(test_negative) >= needed_negative:
                    additional_negative = test_negative.sample(
                        n=needed_negative, random_state=self.random_state
                    )

                    # Add the additional negative samples to top_repo_df
                    top_repo_df = pd.concat(
                        [top_repo_df, additional_negative], ignore_index=True
                    )
                    ...
                    print(
                        f"Added {needed_negative} negative samples from "
                        f"test.csv to top_repos.csv"
                    )
                    print(f"Updated test.csv has {len(test_df)} samples")
                else:
                    print(
                        "Warning: Not enough negative samples in test.csv "
                        "to balance top_repos.csv"
                    )

            # Save the balanced top repository data
            top_repo_df.to_csv(
                os.path.join(self.output_dir, "top_repos.csv"), index=False
            )
